...
const createItem = async (req, res) => {
    ...
    try {
        ...
        const item = await Request.create({ name, email, phone, quantity, notes, service, serviceId, price, reference })
        res.status(200).json(item)
    } catch (error) {
...
const deleteItem = async (req, res) => {
    ...
    const item = await Request.findOneAndDelete({ _id: id })
...
const updateItem = async (req, res) => {
    ...
    const item = await Request.findOneAndUpdate({ _id: id }, {
        ...req.body
    })
